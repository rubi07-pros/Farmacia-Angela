...
define("PASSWORD_PREDETERMINADA", "farmacia");
...
function editarventas($codigo, $nombre, $precio, $cantidad, $subtotal, $id){
    $sentencia = "UPDATE vender SET codigo = ?, nombre = ?, precio = ?, cantidad = ?, subtotal = ? WHERE id = ?";
    $parametros = [$codigo, $nombre, $precio, $cantidad, $subtotal, $id];
    return editar($sentencia, $parametros);
}

function obtenerventasPorId($id){
    $sentencia = "SELECT * FROM vender WHERE id = ?";
    return select($sentencia, [$id])[0];
}
